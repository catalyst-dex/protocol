...
pub fn deposit_liquidity(
    ...
    (amount_a, amount_b) = if pool_creation {
        ...
        let ratio = Decimal::from(pool_a.amount) * Decimal::from(pool_b.amount);
        if pool_a.amount > pool_b.amount {
            (
                (Decimal::from(amount_b) * ratio)
                    .round()
                    .to_u64()
    ...
    let mut liquidity = Decimal::from(amount_a)
        .checked_mul(Decimal::from(amount_b))
...
fn decimal_sqrt(value: Decimal) -> Option<Decimal> {
    ...
    let sqrt = value_f64.sqrt();
    Decimal::try_from(sqrt).ok()
}
