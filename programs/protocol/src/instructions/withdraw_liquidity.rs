...
pub fn withdraw_liquidity(ctx: Context<WithdrawLiquidity>, amount: u64) -> Result<()> {
    ...
    let amount_a = (Decimal::from(amount)
        * Decimal::from(ctx.accounts.pool_account_a.amount)
        / Decimal::from(ctx.accounts.mint_liquidity.supply + MINIMUM_LIQUIDITY))
        .floor()
        .to_u64()
    ...
    let amount_b = (Decimal::from(amount)
        * Decimal::from(ctx.accounts.pool_account_b.amount)
        / Decimal::from(ctx.accounts.mint_liquidity.supply + MINIMUM_LIQUIDITY))
        .floor()
        .to_u64()
        .ok_or(TutorialError::MathError)?;
