...
pub fn swap_exact_tokens_for_tokens(
    ...
    let output = if swap_a {
        let numerator = Decimal::from(taxed_input) * Decimal::from(pool_b.amount);
        let denominator = Decimal::from(pool_a.amount) + Decimal::from(taxed_input);
        (numerator / denominator)
            .round()
            .to_u64()
            .ok_or(TutorialError::MathError)?
    } else {
        let numerator = Decimal::from(taxed_input) * Decimal::from(pool_a.amount);
        let denominator = Decimal::from(pool_b.amount) + Decimal::from(taxed_input);
        (numerator / denominator)
            .round()
            .to_u64()
            .ok_or(TutorialError::MathError)?
